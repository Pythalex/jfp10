def str_array_to_int_array(str_grid):
    ...
    for i in range(len(str_grid)):
        for j in range(len(str_grid[i])):
            array2d[i].append('.' if str_grid[i][j] == '.' else int(str_grid[i][j]))
...
def str_to_array(str_grid):
    ...
    for i in range(len(splitted)):
        for j in range(len(splitted[i])):
            array2d[i].append('.' if splitted[i][j] == '.' else int(splitted[i][j]))
...
def array_to_str(grid_array):
    ...
    for i in range(len(grid_array)):
        array.append("".join([('.' if grid_array[i][j] == '.' else str(grid_array[i][j])) for j in range(len(grid_array[i]))]))
